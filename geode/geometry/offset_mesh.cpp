...
// Can a vertex be split?  That is, does it have at least one interior edge?
static bool can_split(const TriangleTopology& mesh, const VertexId v) {
  const auto e = mesh.halfedge(v);
  return !mesh.is_boundary(e) || e!=mesh.left(mesh.left(e));
}
...
// Pick a vector orthogonal to n and midway between right (r) and left (l).
// We switch between two different formulas based on which of r+l and r-l is bigger.
static TV normal_sum(const TV r, const TV l, const TV n) {
  const auto s = (r+l).projected_orthogonal_to_unit_direction(n),
             d = cross(n,r-l);
  const auto ss = sqr_magnitude(s),
             dd = sqr_magnitude(d);
  return ss >= dd ? s/sqrt(ss)
                  : d/sqrt(dd);
}
...
static Tuple<Array<Vector<int,3>>,Array<TV>> rough_offset_shell_helper(const TriangleTopology& mesh_in,
  ...
  const auto boundary_normal = [&](const HalfedgeId e) {
    ...
    const auto x0 = X[mesh->src(e)],
               x1 = X[mesh->dst(e)],
               dx = x0-x1,
               nf = mesh->normal(X,mesh->face(r));
    {
      const auto n = cross(dx,nf);
      const auto nn = sqr_magnitude(n);
      if (nn)
        return n/sqrt(nn);
    } {
      const auto n = x0+x1-2*X[mesh->opposite(r)];
      const auto nn = sqr_magnitude(n);
      if (nn)
        return n/sqrt(nn);
    }
    return nf.unit_orthogonal_vector();
  };
  ...
  while (work.size()) {
    ...
    if (mesh->is_boundary(v)) {
      ...
      if (   dot(ne,ner) > alpha
        ...
        const auto nr = normal_sum(ner,ne,nv),
                   nl = normal_sum(ne,nel,nv);
        if (det(nv,nr,nl) <= 0)
          goto split;
        B = vec(nr,nl);
      }
